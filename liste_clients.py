...
class Liste_client(Tk):
    ...
    def __init__(self):
        ...
        self.lblcode=Label(self.frame1,text="Code",bg="black",fg="white")
        self.lblcode.place(x=40,y=70,width=100,height=20)
        
        self.var_saicode=StringVar()
        self.saicode=Entry(self.frame1,textvariable=self.var_saicode,bg="gray",fg="white")
        self.saicode.place(x=20,y=100,width=140,height=20)
    ...
    def recherche(self):
        global con
        global cur
        if self.var_saicode.get()!="":
            sql="SELECT id_client,nom, ville, télephone, email FROM client WHERE id_client=%s"
            t=(self.var_saicode.get(),)
            cur.execute(sql,t)
            l=cur.fetchall()
            if len(l)==0:
            ...
            for e  in l:
                ...
                if nom==True and ville==True and tele==True and ema==True:
                    ...
                    messagebox.showinfo("Info","Client n'existe pas")   
        elif self.var_sainom.get()!="":
            sql="SELECT id_client,nom, ville, télephone, email FROM client WHERE nom=%s "
            ...
            sql="SELECT id_client,nom, ville, télephone, email FROM client WHERE ville=%s "
            ...
            sql="SELECT id_client,nom, ville, télephone, email FROM client WHERE télephone=%s "
            ...
            sql="SELECT id_client,nom, ville, télephone, email FROM client WHERE email=%s "
    ...
    def rempl(self):
        global con
        global cur
        sql="SELECT id_client,nom, ville, télephone, email FROM client "
        cur=con.cursor()
        cur.execute(sql)
        l=cur.fetchall()
    ...
    def window(self,n):
        ...
        if n=="modifier":
            ...
            self.var_saicode.set(self.el[0])
    ...
    def ajouter(self):
        ...
        global cur
        if len(self.code())==0:
            sql="INSERT INTO client (id_client, nom, ville, télephone, email) VALUES(%s,%s,%s,%s,%s)"
            t=(self.var_saicode.get(),self.var_sainom.get(),self.var_saiville.get(),self.var_saitele.get(),self.var_saiemail.get())
            cur=con.cursor()
            cur.execute(sql,t)
            con.commit()
    ...
    def modifier(self):
        if len(self.code())>0:
            sql="UPDATE client SET nom=%s,ville=%s,télephone=%s,email=%s WHERE id_client=%s"
            t=(self.var_sainom.get(),self.var_saiville.get(),self.var_saitele.get(),self.var_saiemail.get(),self.var_saicode.get())
